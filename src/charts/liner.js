...
export default class WxLiner extends WxChart {
    ...
    constructor(id, config) {
        ...
        me.wxCrossScale = new WxCrossScale(me.xAxis, me.yAxis);
    ...
    _getLegendConfig() {
        ...
        if (!legendsConfig) {
            ...
            legendsConfig = legendsConfig.map(legend => {
                return extend(true, { 'key': defaultKey }, legend);
            })
        }
    ...
    draw() {
        ...
        let rColors = randomColor(
            extend(true, {}, color, {count: me.legends.length})
        );
        let legendDatasets = [];
        me.legends.forEach(function(legend, index){
            ...
            extend(legend, {
                ...
                spanGaps: legend.spanGaps || me.chartConfig.spanGaps,
                tension: legend.tension || me.chartConfig.tension,
                fillStyle: legend.fillStyle || rColors[index],
                strokeStyle: legend.strokeStyle || rColors[index],
                lineWidth: legend.lineWidth || me.chartConfig.lineWidth,
                lineJoin: legend.lineJoin || me.chartConfig.lineJoin,
                fillArea: legend.fillArea || me.chartConfig.fillArea,
                fillAlpha: legend.fillAlpha || me.chartConfig.fillAlpha,
            });

            legendDatasets.push(legend);
        });
    ...
    _drawLine(lineData) {
        ...
        let { display, spanGaps, tension, lineWidth, lineJoin, fillStyle, strokeStyle, fillArea, fillAlpha } = legend;
        ...
        let lineToPoint = function(pre, p, next) {
            if (!tension || tension === 0) {
                ...
                if (!pre) {
                    ...
                    ctx.bezierCurveTo(
                        pre.controlPoints.next.x,
                        pre.controlPoints.next.y,
        ...
        let getNextPoint = function(dataset, index, spanGaps) {
            ...
            if (!nextDate.point) {
                if (!!spanGaps) return getNextPoint(dataset, index+1, spanGaps);
                else return;
            }
        ...
        dataset.forEach((d, index) => {
           ...
           let { pointBorderColor, pointBorderWidth, pointRadius, pointStyle, label } = data;
            ...
            pointStack.push({
                point, pointBorderColor, pointBorderWidth, pointRadius, pointStyle
            });
    ...
    yScaleAxisDatas(area) {
        ...
        let {max, min} = this.datasets.reduce((pre, cur) => {
            let {max, min} = pre;
            if (cur.display) {
                let curValue = me.legends
                    .map((legend) => { if (legend.key) { return cur[legend.key] || 0 } })
                    .concat(max, min);
